...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj4_integration): Update the following line
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.NL);
        // find the right root leaf to go to and then go to the leftmost leaf in that.
    ...
    private void splitRootCode(Optional<Pair<DataBox, Long>> o) {
        // we need to split the root.
        Pair<DataBox, Long> splitCause = o.get();
        DataBox newKey = splitCause.getFirst();
        Long newPageNum = splitCause.getSecond();
        // CREATE A NEW ROOT!
        List<DataBox> newRootKeys = new ArrayList<>(Arrays.asList(newKey));
        List<Long> newRootChildren = new ArrayList<>(Arrays.asList(root.getPage().getPageNum(), newPageNum));
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // SHOULD PARALLEL INNER NODE'S .
        if (this.root.getLeftmostLeaf().scanAll().hasNext()) {
            throw new BPlusTreeException("Bulk loading is only allowed on empty trees");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> overflow = root.bulkLoad(data, fillFactor);
            if (overflow.isPresent()) {
                splitRootCode(overflow);
            }
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        // iterator for that leaf over the records
        private Iterator<RecordId> leafIterator;
        // our has next op
        boolean another;

        public BPlusTreeIterator(LeafNode leftMost, Iterator<RecordId> r) {
            this.currentLeaf = leftMost;
            this.leafIterator = r;
            another = true;
            nextCheck();
        }
        ...
        protected void nextSibling() {
            ...
            if (sibling.isPresent()) {
                this.currentLeaf = sibling.get();
                this.leafIterator = currentLeaf.scanAll();
                nextCheck();
            } else {
                another = false;
            }
        ...
        public RecordId next() {
            ...
            RecordId nextRec = leafIterator.next();
            nextCheck();
            return nextRec;
        }
