...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        // go through keys, find the key which is closest of the leafs available.
        if (children.size() == 0) {
            return null;
        }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        int pos = numLessThanEqual(key, keys);
        Optional<Pair<DataBox, Long>> split = getChild(pos).put(key, rid);
        if (split.isPresent()) {
            // we have a new right key node.
            Pair<DataBox, Long> ourPair = split.get();
            int newPos = InnerNode.numLessThan(key, keys);
            keys.add(newPos, ourPair.getFirst());
            children.add(newPos + 1, ourPair.getSecond());
            if (keys.size() <= 2 * metadata.getOrder()) {
                // inserted in right place, we r done.
                sync();
                return Optional.empty();
            } else {
                // we are going to go over length, we need to split.
                // Let's start by inserting the value first.
                DataBox middle_k = keys.get(metadata.getOrder());
                List<DataBox> left_k = new ArrayList(keys.subList(0, metadata.getOrder()));
                List<Long> left_r = new ArrayList(children.subList(0, metadata.getOrder() + 1));


                InnerNode newNode = new InnerNode(metadata, bufferManager,
                        new ArrayList(keys.subList(metadata.getOrder() + 1, 2 * metadata.getOrder() + 1)),
                        new ArrayList(children.subList(metadata.getOrder() + 1, metadata.getOrder() * 2 + 2)),
                        treeContext);
                Long newNodePageNum = newNode.getPage().getPageNum();
                this.keys = left_k;
                this.children = left_r;
                sync();
                return Optional.of(new Pair<>(middle_k, newNodePageNum));
            }
        } else {
            return Optional.empty();
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        /*
           If has a child:
           - get the right most child, then bulkload it with all your data at the fill factor
           while the rightmost child is yielding splits:
           - add that to our current key/children pairs
           - if our current key/children pairs > 2d, then arrange for a split.
         */
        int twoD = 2* metadata.getOrder();
        while (data.hasNext() && keys.size() <= twoD) {
            BPlusNode currentRight = getChild(children.size() - 1);
            Optional<Pair<DataBox, Long>> loadRight = currentRight.bulkLoad(data, fillFactor);
            if (loadRight.isPresent()) {
                keys.add(loadRight.get().getFirst());
                children.add(loadRight.get().getSecond());
            }
        }
        if (keys.size() > twoD) {
            DataBox toPush = keys.get(twoD/2);
            InnerNode toRet = new InnerNode(metadata, bufferManager, new ArrayList<DataBox>(keys.subList(twoD/2 + 1, twoD/2 + 1)), new ArrayList<Long>(children.subList(twoD/2 + 1, twoD/2 + 2)), treeContext);
            this.keys = new ArrayList<>(keys.subList(0, twoD/2));
            this.children = new ArrayList<>(children.subList(0, twoD/2));
            sync();
            return Optional.of(new Pair<>(toPush, toRet.getPage().getPageNum()));
        } else {
            sync();
            return Optional.empty();
        }
