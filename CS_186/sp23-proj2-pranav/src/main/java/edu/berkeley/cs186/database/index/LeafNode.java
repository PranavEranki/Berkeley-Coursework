...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // no duplicates - since leaf, check if the key matches up to existing keys.
        // if duplicate, raise exception
        if (keys.contains(key)) {
            throw new BPlusTreeException("No duplicate keys. You tried entering " + key.toString());
        }

        // no overflow when put? Return optional.empty
        if (keys.size() <= 2 * metadata.getOrder() - 1) {
            ...
            // there will be overflow - we will go over 2*order if we insert. We need to split.
            // overflow = split_key and right_node_page_num

            int pos = InnerNode.numLessThan(key, keys);
            List<DataBox> left_k;
            List<DataBox> right_k;
            List<RecordId> left_r;
            List<RecordId> right_r;
            if (pos > metadata.getOrder() - 1) {
                // new node goes on the right.
                left_k = new ArrayList(keys.subList(0, metadata.getOrder()));
                right_k = new ArrayList(keys.subList(metadata.getOrder(), 2*metadata.getOrder()));
                left_r = new ArrayList(rids.subList(0, metadata.getOrder()));
                right_r = new ArrayList(rids.subList(metadata.getOrder(), 2*metadata.getOrder()));
                pos = pos-metadata.getOrder();
                right_k.add(pos, key);
                right_r.add(pos, rid);
            } else {
                // new node goes on the left.
                left_k = new ArrayList(keys.subList(0, metadata.getOrder() - 1));
                right_k = new ArrayList(keys.subList(metadata.getOrder() - 1, 2*metadata.getOrder()));
                left_r = new ArrayList(rids.subList(0, metadata.getOrder() - 1));
                right_r = new ArrayList(rids.subList(metadata.getOrder() - 1, 2*metadata.getOrder()));
                left_k.add(pos, key);
                left_r.add(pos, rid);
            }
            LeafNode newRight = new LeafNode(metadata, bufferManager, right_k, right_r, rightSibling, treeContext);
            Optional<Long> pageOfRight = Optional.of(newRight.getPage().getPageNum());
            this.keys = left_k;
            this.rids = left_r;
            this.rightSibling = pageOfRight;
            sync();

            // keep the first d nodes in this
            // create a new right node with the
            Pair x = new Pair<>(right_k.get(0), pageOfRight.get());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        /*
        Get the max capacity and do not USE the iterator beyond that capacity.
        if the current one will overflow, you need to create the next LeafNode and get
        the rest of the data to fill to that node.
         */
        int twoD = 2* metadata.getOrder();
        int max = (int) Math.ceil((twoD*fillFactor));
        ...
        if (data.hasNext()) {
            ...
            List<DataBox> rightKeys = new ArrayList<>(Arrays.asList(next.getFirst()));
            List<RecordId> rightRIds = new ArrayList<>(Arrays.asList(next.getSecond()));
            LeafNode right = new LeafNode(metadata, bufferManager, rightKeys, rightRIds, Optional.empty(), treeContext);
            long pageNum = right.page.getPageNum();
            this.rightSibling = Optional.of(pageNum);
            sync();
            right.sync();
            DataBox identifier = rightKeys.get(0);
            return Optional.of(new Pair<>(identifier, pageNum));
        }
        sync();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // byte 1 = literal value 1
        byte nodeType = buf.get();
        assert(nodeType == (byte) 1);
        // byte 2-9 = page id of right sibling or -=1 if it doesn't exist
        long right = buf.getLong();
        int numPairs = buf.getInt();
        List<DataBox> keys = new ArrayList<>();
        List<RecordId> children = new ArrayList<>();
        for (int i = 0 ; i < numPairs ; i ++) {
            DataBox box = DataBox.fromBytes(buf, metadata.getKeySchema());
            RecordId desired = RecordId.fromBytes(buf);
            keys.add(box);
            children.add(desired);
        }

        if (right == -1){
            return new LeafNode(metadata, bufferManager, page, keys, children, Optional.empty(),treeContext);
        } else {
            return new LeafNode(metadata, bufferManager, page, keys, children, Optional.of(right), treeContext);
        }
