...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        Collections.sort(arr, this.comparator);
        Run run = makeRun();
        run.addAll(arr);
        return run;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        PriorityQueue<Pair<Record, Integer>> pq = new PriorityQueue<Pair<Record, Integer>>(runs.size(),
                (a,b) -> comparator.compare(a.getFirst(), b.getFirst()));
        ArrayList<Iterator<Record>> iters = new ArrayList<>();
        int i = 0;
        for(Run r : runs) {
            iters.add(r.iterator());
            pq.offer(new Pair<>(iters.get(i).next(), Integer.valueOf(i++)));
        }
        while(!pq.isEmpty()) {
            Pair<Record, Integer> cur = pq.poll();
            Record minRec = cur.getFirst();
            Integer iNum = cur.getSecond();
            mergedRun.add(minRec);
            if(iters.get(iNum).hasNext()) {
                pq.add(new Pair<>(iters.get(iNum).next(), iNum));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        while (curRun < runs.size()) {
            i = curRun;
            ArrayList<Run> curBatch = new ArrayList<>();
            for (i = curRun; i < runs.size() && i < curRun + numBuffers-1; i++) {
                curBatch.add(runs.get(i));
            }
            retRun.add(mergeSortedRuns(curBatch));
            curRun += numBuffers-1;
        }
        return retRun;
    }
    ...
    public Run sort() {
        ...
        // divide into numBuffer runs and sort them all
        List<Run> runs = new ArrayList<>();
        while(sourceIterator.hasNext()) {
            Iterator<Record> runIter = QueryOperator.getBlockIterator(
                    sourceIterator, outputSchema, numBuffers);
            Run curRun = sortRun(runIter);
            runs.add(curRun);
        }

        // keep merging till one run left
