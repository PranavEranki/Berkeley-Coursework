...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        // determine cost of a seq scan for the table
        // for every index that can be used, find costof an index scna
        // kep track of min cost operation, push down eligible single predicates
        // loop 1 : for each eligible column
        // index scan operator java - table name,  (>, <, <=, >=, =) and a value.
        // column name must be one with an index built into it
        QueryOperator newMinOp;
        for (int i = 0 ; i < eligibleIndices.size(); i ++) {
            SelectPredicate curr = this.selectPredicates.get(eligibleIndices.get(i));
            newMinOp = new IndexScanOperator(this.transaction, table, curr.column, curr.operator, curr.value);
            int cost = newMinOp.estimateIOCost();
            if (cost < minCost) {
                minCost = cost;
                minOp = newMinOp;
                minOp = addEligibleSelections(minOp, eligibleIndices.get(i));
            }
        }
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Set<Set<String>> asArr = prevMap.keySet();
        // For each set of tables in prevMap
        for (Set<String> currTables: prevMap.keySet() ) { // int i = 0 ; i < prevMap.size(); i++) {
            QueryOperator currQ = prevMap.get(currTables);
            //   For each join predicate listed in this.joinPredicates
            for (int j = 0 ; j < this.joinPredicates.size(); j ++) {
                JoinPredicate curr = this.joinPredicates.get(j);
                String leftSideT = curr.leftTable;
                String rightSideC = curr.rightColumn;
                String rightSideT = curr.rightTable;
                String leftSideC = curr.leftColumn;
                QueryOperator newOp;
                Set<String> ret = new HashSet<>(currTables);
                if (currTables.contains(leftSideT) && !currTables.contains(rightSideT)) {
                    // CASE 1 COVERED
                    ret.add(rightSideT);
                    newOp = pass1Map.get(Collections.singleton(rightSideT));
                    if (newOp != null) {
                        if (!result.containsKey(Collections.singleton(rightSideT)) ||
                                result.get(Collections.singleton(rightSideT)).estimateIOCost() > minCostJoinType(currQ, newOp, leftSideC, rightSideC).estimateIOCost()) {
                            result.put(ret, minCostJoinType(prevMap.get(currTables), newOp, leftSideC, rightSideC));
                        }
                    }

                } else if (currTables.contains(rightSideT) && !currTables.contains(leftSideT)) {
                    // CASE 2 COVERED
                    ret.add(leftSideT);
                    newOp = pass1Map.get(Collections.singleton(leftSideT));
                    if (newOp != null) {
                        if (!result.containsKey(Collections.singleton(leftSideT)) ||
                                result.get(Collections.singleton(leftSideT)).estimateIOCost() > minCostJoinType(currQ, newOp, rightSideC, leftSideC).estimateIOCost()) {
                            result.put(ret, minCostJoinType(prevMap.get(currTables), newOp, rightSideC, leftSideC));
                        }
                    }
                } else {
                    continue;
    ...
    public Iterator<Record> execute() {
        this.transaction.setAliasMap(this.aliases);
        // TODO(proj3_part2): implement
        HashMap<Set<String>, QueryOperator> lowestCostOperator = new HashMap<Set<String>, QueryOperator>();
        for (String currTable:this.tableNames) {
            QueryOperator op = minCostSingleAccess(currTable);
            Set<String> ourSet = new HashSet<>(Arrays.asList(currTable));
            lowestCostOperator.put(ourSet, op);
        }
        Map<Set<String>, QueryOperator> singleAccesses = new HashMap<Set<String>, QueryOperator>(lowestCostOperator);
        for (int i = 2; i <= tableNames.size(); i++) {
            lowestCostOperator = (HashMap<Set<String>, QueryOperator>) minCostJoins(lowestCostOperator, singleAccesses);
        }
        this.finalOperator = minCostOperator(lowestCostOperator);
        addGroupBy();
        addProject();
        addSort();
        addLimit();
