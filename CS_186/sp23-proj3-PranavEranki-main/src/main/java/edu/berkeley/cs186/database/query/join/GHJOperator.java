...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        // build
        for (Record rec : buildRecords) {
            DataBox buildJoinValue = rec.getValue(buildColumnIndex);
            if(!hashTable.containsKey(buildJoinValue))
                hashTable.put(buildJoinValue, new ArrayList<>());
            hashTable.get(buildJoinValue).add(rec);
        }

        // probe
        for (Record pRec : probeRecords) {
            DataBox probeJoinVal = pRec.getValue(probeColumnIndex);
            if(!hashTable.containsKey(probeJoinVal)) continue;
            for (Record bRec : hashTable.get(probeJoinVal)) {
                Record joinedRecord;
                if (probeFirst) {
                    joinedRecord = pRec.concat(bRec);
                } else {
                    joinedRecord = bRec.concat(pRec);
                }
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            for (int j = 0; j < rightPartitions.length; j++) {
                if (leftPartitions[i].getNumPages() <= usableBuffers) {
                    buildAndProbe(leftPartitions[i], rightPartitions[j]);
                } else if(rightPartitions[j].getNumPages() <= usableBuffers) {
                    buildAndProbe(leftPartitions[i], rightPartitions[j]);
                } else {
