...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(0, X_RANGE + Y_RANGE - 1 + MAX_NOISE):
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        """
        PSEUDOCODE
        1. Get uncond and cond
        2. Remove variable if in uncond, get the other variables for Factor
        2. Join and eliminate
            - Cut the column, sum where the variables not being cut r same and the column being cut is diff
            - discard if only one unconditioned var

        NOTES:
        - joining and eliminating variables.
        - 
        CODE:

        
        """
        existing = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for hidden in eliminationOrder:
            new = joinFactorsByVariable(existing, hidden)
            existing = new[0]
            temp = new[1]
            trueunconditioned = temp.unconditionedVariables()
            if len(trueunconditioned) > 1:
                existing.append(eliminate(temp, hidden))
        return normalize(joinFactors(existing))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        denominator = self.total()
        whetherToReturn = random.uniform(0, denominator)
        increasingProbOfSample = 0
        for n, pOfSample in self.items():
            increasingProbOfSample += pOfSample
            if increasingProbOfSample > whetherToReturn:
                return n
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        "*** YOUR CODE HERE ***"
        """
        observation = noisy reading of dist to ghost
        Pacman's Pos
        Ghost's Poss
        Pos of ghost jail
        Returns P(noisy distance given pacman, ghost pos)"""
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Each belief for each position is updated accoridngly to the probability of ghost being in that square
        # given what we know previously and the new reading.
        ...
        # No need to normalize, done below.
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # update the belief at every pos on themap after on time step.
        
        newDD = DiscreteDistribution()
        for pos in self.allPositions:
            # action distribution for the ghosts
            ghostDist = self.getPositionDistribution(gameState, pos).items()
            for potentialPos, p in ghostDist:
                newDD[potentialPos] += self.beliefs[pos] * p
        self.beliefs = newDD
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # poss = self.legalPositions
        # newP = []
        # for i in range(self.numParticles):
        #     newP.append(random.randint(0, self.numParticles/len(poss)))
        # self.particles = newP
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        notNormalized = DiscreteDistribution()
        for i in range(len(self.particles)):
            notNormalized[self.particles[i]] += 1.0
        notNormalized.normalize()
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # weight distr over self.particles, weight = prob of observation given position and location
        # resample from this weighted distr and construct new list of particles.
        distry = DiscreteDistribution()
        for pa in self.particles:
            distry[pa] += self.getObservationProb(observation, gameState.getPacmanPosition(),
            pa, self.getJailPosition())

        # not the issue
        if distry.total() == 0:
            # special case, initialize uniformly
            self.initializeUniformly(gameState)
            return
            # distry = self.getBeliefDistribution()
        
        distry.normalize()
        self.particles=[]
        for i in range(self.numParticles):
            self.particles.append(distry.sample())
    ...
    def elapseTime(self, gameState):
        ...
        for elem in self.particles:
            if elem not in sampled.keys():
                sampled[elem] = self.getPositionDistribution(gameState, elem)
            self.particles[count] = sampled[elem].sample()
            count = count + 1
        "*** END YOUR CODE HERE ***"
