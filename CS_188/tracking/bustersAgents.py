...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        closestGhost = None
        for potentialLocationDist in livingGhostPositionDistributions:
            maxProb = potentialLocationDist.argMax()
            dist = self.distancer.getDistance(maxProb, pacmanPosition)
            if (dist < closestDist):
                closestDist = dist
                closestGhost = maxProb
            # distToMostLikely = # dist to 
        
        # Bring pacman to the closest ghost
        maximumUtility = 100000
        toRet = None
        for possibleAction in legal:
            newDistance = self.distancer.getDistance(
                closestGhost, Actions.getSuccessor(pacmanPosition, possibleAction))
            if (newDistance < maximumUtility):
                maximumUtility = newDistance
                toRet = possibleAction
