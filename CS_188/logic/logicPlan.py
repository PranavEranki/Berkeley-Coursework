...
def sentence2() -> Expr:
    ...
    three = ~(B & ~C) >> A
    four = ~D >> C
    result = conjoin([one, two, three, four])
...
def sentence3() -> Expr:
    ...
    one = alive_1 % disjoin([conjoin([alive_0, ~killed_0]), conjoin([~alive_0, born_0])])
    two = ~(alive_0 & born_0)
    three = born_0
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    a = itertools.combinations(literals, 2)
    b = [~x | ~y for x, y in a]
    return conjoin(b)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    #there's only one pacman
    pac_pos = []
    for x, y in non_outer_wall_coords:
        pac_pos.append(PropSymbolExpr(pacman_str, x, y, time=t))
    exac_one = exactlyOne(pac_pos)
    pacphysics_sentences.append(exac_one)

    #there's only one direction
    actions = []
    for d in DIRECTIONS:
        actions.append(PropSymbolExpr(d, time=t))
    exac_one = exactlyOne(actions)
    pacphysics_sentences.append(exac_one)

    #sensorModel
    if sensorModel: #check if sensorModel is None
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
        
    #successorAxioms
    if t > 0:
        if successorAxioms: #checking if successorAxioms is None
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    #point 1
    # print("checkpoint1")
    time_0 = pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid, None, allLegalSuccessorAxioms)
    # print("checkpoint1.111")
    time_1 = pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid, None, allLegalSuccessorAxioms)
    # print("checkpoint1.1")
    KB.append(time_0)
    KB.append(time_1)
    #point 2
    # print("checkpoint2")
    current_location = PropSymbolExpr(pacman_str, x0, y0, time=0)
    KB.append(current_location)
    #point 3
    # print("checkpoint3")
    ...
    #query
    # print("checkpoint4")
    KB1, KB2 = KB.copy(), KB.copy()
    KB1.append(PropSymbolExpr(pacman_str, x1, y1, time=1))
    KB2.append(~PropSymbolExpr(pacman_str, x1, y1, time=1))
    model_1 = findModel(conjoin(KB1))
    model_2 = findModel(conjoin(KB2))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            pac_pos.append(PropSymbolExpr(pacman_str, x, y, time=t))
        exact_one = exactlyOne(pac_pos)
        KB.append(exact_one)
        print("KB Is :")
        print((type(expr), expr) for expr in KB if not isinstance(expr, Expr))
        #check satisfying assignment
        goal_assertion = PropSymbolExpr(pacman_str, xg, yg, time=t)
        KB_current = KB.copy() #cannot be at the goal state every timestep
        KB_current.append(goal_assertion)
        satisfiability = findModel(conjoin(KB_current))
        if satisfiability:
            extractActionSequence(satisfiability, actions)

        #exactly one action per t
        action = []
        for d in actions:
            action.append(PropSymbolExpr(d, time=t))
        only_one = exactlyOne(action)
        KB.append(only_one)
        #transition model sentence
        if t > 0:
            for x, y in non_wall_coords:
                axioms = [pacmanSuccessorAxiomSingle(x, y, t, walls)]
                KB.append(axioms)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        # Add pacphysics, action, and percept info to KB
        axx = pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms)
        KB.append(axx)
        KB.append(PropSymbolExpr(agent.actions[t],t))
        # Percepts
        four_bit = fourBitPerceptRules(t, agent.getPercepts())
        KB.append(four_bit)
        ...
        for x,y in non_outer_wall_coords:
            if (entails(conjoin(KB), PropSymbolExpr(pacman_str, x,y, time=t))):
                provable_locations.append((x,y))
                print("Possible location added")
            if (entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x,y, time=t))):
                not_possible.append((x,y))
                print("impossible location added")
            else:
                possible_locations.append((x,y))
            if (x,y) in provable_locations and (x,y) in not_possible:
                print("ERROR!! X,Y IN NOT POSSIBLE AND POSSIBLE!")
            for x,y in provable_locations:
                KB.append(PropSymbolExpr(pacman_str, x,y, time=t))
            for x,y in not_possible:
                KB.append(~PropSymbolExpr(pacman_str, x,y, time=t))
...
def mapping(problem, agent) -> Generator:
    ...
    walls_grid = problem.walls
    walls_list = walls_grid.asList()
    if ((pac_x_0,pac_y_0) not in walls_list):
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        axx = pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, SLAMSuccessorAxioms)
        KB.append(axx)
        KB.append(PropSymbolExpr(agent.actions[t],time = t))
        four_bit = numAdjWallsPerceptRules(t, agent.getPercepts())
        KB.append(four_bit)
        for (x,y) in non_outer_wall_coords:
            print("We're at:", x,y)
            if (entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))):
