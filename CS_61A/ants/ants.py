...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        poss = 0
        while not p.is_hive:
            if (p.bees and poss >= self.min_range and poss <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        # damage others
        damageTodo = amount + (self.damage if amount >= self.health else 0)
        for beee in self.place.bees[:]:
            beee.reduce_health(damageTodo)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if (self.place.bees and self.chew_countdown == 0):
            randomBeeToEat = choose_bee(self.place.bees)
            randomBeeToEat.reduce_health(randomBeeToEat.health)
            print("DEBUG: " + str(randomBeeToEat))
            self.chew_countdown = self.chew_duration
        else:
            self.chew_countdown = (self.chew_countdown - 1 if self.chew_countdown > 0 else 0)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for beee in self.place.bees[:]:
            beee.reduce_health(self.damage)
