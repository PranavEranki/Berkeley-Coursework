...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        ...
        for (int i = s ; i < ARRAY_SIZE ; i += gap){
            z[i] = x[i] + y[i];
        }
    }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        int size = ARRAY_SIZE / omp_get_num_threads();
        int currThread = omp_get_thread_num();
        int end;
        if (currThread == omp_get_num_threads()-1) {
            end = ARRAY_SIZE;
        } else {
            end = currThread*size + size;
        }
        for(int i = currThread * size; i < end; i++) { // changed from ++i
            z[i] = x[i] + y[i];
        }
    }
}
