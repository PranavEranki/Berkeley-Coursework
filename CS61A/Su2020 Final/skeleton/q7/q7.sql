...
CREATE TABLE parta AS
    SELECT DVD.name 
    from DVD JOIN inventory on DVD.name = inventory.name
    WHERE DVD.year >= 2010
    AND (inventory.total - inventory.available - inventory.rented) > 0
    ORDER BY (inventory.total - inventory.available - inventory.rented) DESC;
...
CREATE TABLE partb AS
    SELECT DVD.ratings, COUNT(*)
    from DVD 
    JOIN july_rental on DVD.name = july_rental.name
    GROUP BY DVD.ratings
    WHERE (july_rental.return_date - july_rental.rented_date) > 3
    HAVING COUNT(*) > 1;
