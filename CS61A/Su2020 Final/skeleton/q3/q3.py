...
def same_length(t, desired):
    ...
    if not desired:
        t.branches = []
    elif not len(t.branches):
        t.branches = [Tree("hello")]
    for br in t.branches:
        same_length(br, desired-1)
...
def shortest_no_hello(t):
    ...
    [4, 10]
    """
    if all(br.label for br in t.branches == "hello"):
        return [t.label]
    return [t.label] + min([shortest_no_hello(br) for br in t.branches], key=len)
