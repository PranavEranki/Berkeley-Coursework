...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        checki = scheme_eval(expr.first, env)
        validate_procedure(checki)
        return scheme_apply(checki, expr.rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 4
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        curr = self
        while curr.parent:
            if not curr.bindings.get(symbol) == None:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        frame_to_return = self.env.make_child_frame(self.formals, args)
...
def make_let_frame(bindings, env):
    ...
    values = bindings.map(lambda item: item.rest.first)
    values = values.map(lambda item: scheme_eval(item, env))
